...
def create_window_linelist(seg_begins: list[float], seg_ends: list[float], line_list_files_draft: list[str],
						   new_path_name: str, molecules_flag: str):
	lbl = False
	...
	if not lbl:
		if not os.path.exists(new_path_name):
			os.makedirs(os.path.join(f"{new_path_name}", ''))
		#else:
...
def write_lines(all_lines_to_write: dict[list[str]], elem_line_1_to_save: str, elem_line_2_to_save: str, new_path_name: str, line_list_number: float):
	for key in all_lines_to_write:
		new_linelist_name: str = os.path.join(f"{new_path_name}", f"linelist-{line_list_number}.bsyn")
